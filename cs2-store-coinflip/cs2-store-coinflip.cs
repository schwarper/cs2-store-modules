...
public class Store_CoinFlip : BasePlugin, IPluginConfig<Store_CoinFlipConfig>
{
    ...
    public void Command_DuelCoinFlip(CCSPlayerController? player, CommandInfo info)
    {
        ...
        {
            info.ReplyToCommand(Config.Tag + Localizer["Duel feature is disabled"]);
            return;
        }
        ...
        {
            info.ReplyToCommand(Config.Tag + Localizer["Cannot challenge yourself"]);
            return;
        }

        if (targetPlayer == null || !targetPlayer.IsValid || targetPlayer.IsBot || targetPlayer.IsHLTV)
        {
            info.ReplyToCommand(Config.Tag + Localizer["Invalid target player"]);
            return;
        }
        ...
        {
            info.ReplyToCommand(Config.Tag + Localizer["Min coinflip", Config.MinDuel]);
            return;
        }

        if (credits > Config.MaxDuel)
        {
            info.ReplyToCommand(Config.Tag + Localizer["Max coinflip", Config.MaxDuel]);
            return;
        }

        if (StoreApi.GetPlayerCredits(player) < credits)
        {
            info.ReplyToCommand(Config.Tag + Localizer["No enough credits"]);
            return;
        }

        if (StoreApi.GetPlayerCredits(targetPlayer) < credits)
        {
            info.ReplyToCommand(Config.Tag + Localizer["Target player has no enough credits"]);
            return;
        }
    ...
    {
        challenger.PrintToChat(Config.Tag + Localizer["Duel request sent", target.PlayerName, credits]);
        target.PrintToChat(Config.Tag + Localizer["Duel request", challenger.PlayerName, credits]);
    ...
    public void Command_AcceptDuel(CCSPlayerController? player, CommandInfo info)
    {
        ...
        {
            info.ReplyToCommand(Config.Tag + Localizer["No duel invitations"]);
            return;
        }
    ...
    public void Command_DeclineDuel(CCSPlayerController? player, CommandInfo info)
    {
        ...
        {
            player.PrintToChat(Config.Tag + Localizer["You declined the duel"]);
        }
        else
        {
            info.ReplyToCommand(Config.Tag + Localizer["No duel invitations"]);
    ...
    public void StartDuel(CCSPlayerController target, DuelInfo duelInfo)
    {
        ...
        {
            target.PrintToChat(Config.Tag + Localizer["No enough credits"]);
            challenger.PrintToChat(Config.Tag + Localizer["No enough credits"]);
            return;
        }
        ...
        winner.PrintToChat(Config.Tag + Localizer["Duel win message", loser.PlayerName, credits * multiplier]);

        loser.PrintToChat(Config.Tag + Localizer["Duel lose message", winner.PlayerName, credits]);
    ...
    public void Command_CoinFlip(CCSPlayerController? player, CommandInfo info)
    {
        ...
        {
            info.ReplyToCommand(Config.Tag + Localizer["Coinflip feature is disabled"]);
            return;
        }

        if (Utilities.FindAllEntitiesByDesignerName<CCSGameRulesProxy>("cs_gamerules").First().GameRules!.WarmupPeriod)
        {
            info.ReplyToCommand(Config.Tag + Localizer["Cannot join during warmup"]);
            return;
        }

        if (GlobalCoinFlip.Values.Any(dict => dict.ContainsKey(player)))
        {
            info.ReplyToCommand(Config.Tag + Localizer["You are already in"]);
            return;
        }
        ...
        {
            info.ReplyToCommand(Config.Tag + Localizer["Min coinflip", Config.MinCoinFlip]);
            return;
        }

        if (credits > Config.MaxCoinFlip)
        {
            info.ReplyToCommand(Config.Tag + Localizer["Max coinflip", Config.MaxCoinFlip]);
            return;
        }
    ...
    public void JoinCoinFlip(CCSPlayerController player, CommandInfo info, int credits, string option)
    {
        ...
        Server.PrintToChatAll(Config.Tag + Localizer["Join coinflip", player.PlayerName, credits, Localizer[option]]);
    }
    ...
    public HookResult OnRoundStart(EventRoundStart @event, GameEventInfo info)
    {
        ...
        if (Config.EnableCoinflipStartOfRound)
        {
            Server.PrintToChatAll(Config.Tag + Localizer["Announce coinflip"]);
            Pay();
        }
    ...
    public void GiveCreditsToWinner(string option)
    {
        ...
        Server.PrintToChatAll(Config.Tag + Localizer["Winner coinflip", Localizer[option]]);
        Pay(option);
